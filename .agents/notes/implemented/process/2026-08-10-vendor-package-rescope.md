...
Cordis-owned remote event names are runtime package namespaces, so `cordis/...` becomes `@deepseek-ai/cordis/...` and the catalog projector treats `@scope/package` as one event scope. UI locale namespaces, input-trigger source names, and preset ids remain product identifiers. Files containing only those values are exempted; `ui-cordis/src/client/index.ts` mixes both classes, so one exact edit renames its four remote events while the generic pass is disabled for that file. Postconditions pin both sides of this boundary.
