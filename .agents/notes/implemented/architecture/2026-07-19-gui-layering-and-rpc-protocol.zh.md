...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/electron` 通过 OS 分配的私有回环监听复用 Web 组合；目前未实现原生 IPC 载体。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 浏览器访问与 Electron 回环壳；零 workspace 依赖（注册表经结构注入到达）；原生 Electron IPC 尚未实现 |
