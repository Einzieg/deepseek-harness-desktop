...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/electron` currently reuses the same Web composition over a private OS-assigned loopback listener; a future native carrier may replace that physical boundary with IPC.
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser access and the current Electron loopback shell; zero workspace dependencies (the registry arrives by structural injection); native Electron IPC is not implemented |
