...
The package knows no harness concepts and serves no files: the `/api` HTTP bridge and downlink WebSockets are routes owned by the connection plugin, plugin bundles and the HMR event stream are routes owned by the modules/hmr plugins, and dist serving belongs to the fallback owner. The upgrade handler owns the protocol handshake and connection contents; the webserver only delivers the raw socket and request. `host` accepts only `127.0.0.1` (default posture) and `0.0.0.0` (deliberate network exposure). The Electron shell reuses this browser carrier through a private OS-assigned `127.0.0.1` listener; no native `file://` plus IPC carrier is implemented. This package never prints; the URL line belongs to the shell.
